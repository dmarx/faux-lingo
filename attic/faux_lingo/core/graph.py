...
class ColoredGraph:
    ...
    def build_color_transitions(self) -> None:
        ...
        if self.config.random_color_transitions:
            ...
            for i in range(n_colors):
                ...
                alphas = np.array(
                    [
                        np.exp(-((i - j) ** 2) / (2 * self.config.sigma**2))
                        + self.config.epsilon
        ...
        logger.debug(
            "Built color transition matrix of shape {}", self.color_matrix.shape
        )
    ...
    def build_transition_matrix(self) -> None:
        ...
        for i in range(self.config.vocab_size):
            ...
            target_colors = self._np_rng.choice(
                available_colors, size=self.config.avg_degree, replace=False, p=p_colors
            )
        ...
        logger.debug(
            "Built transition matrix of shape {}", self.transition_matrix.shape
        )
    ...
    def get_steady_state(self, tol: float = 1e-8, max_iter: int = 1000) -> np.ndarray:
        ...
        logger.warning(
            "Steady state computation did not converge after {} iterations", max_iter
        )
        return pi
