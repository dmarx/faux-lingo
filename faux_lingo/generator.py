...
def validate_shapes(tensors: Dict[str, torch.Tensor], expected_shapes: Dict[str, Tuple[int, ...]]):
    """
...
class LanguageParams:
    ...
    def save(self, path: Union[str, Path]):
        """Save language parameters using tensorizer."""
        data = {
        ...
        tensorizer.save(data, path)
    
    @classmethod
    def load(cls, path: Union[str, Path], device: str = 'cuda'):
        """Load language parameters using tensorizer."""
        data = tensorizer.load(path, map_location=device)
        return cls(
...
class ProbColorConstrainedGenerator:
    ...
    def __init__(
        ...
        self.boundaries = torch.zeros(self.n_colors + 1, dtype=torch.long, device=device)
    ...
    def generate_transitions(
        ...
        if mixtures is not None:
            validate_shapes({
                ...
                'mixtures': (batch_size, self.n_topics)
            })
        else:
        ...
        Q = self.topic_vectors.expand(batch_size, -1, -1)
        M = torch.bmm(torch.bmm(Q, Lambda), Q.transpose(-2, -1))
        ...
        M = torch.where(self.transition_mask > 0, 
    ...
    def sample_sequences(
        ...
        if start_color is not None:
            ...
            sequences[:, 0] = torch.randint(start, end, (batch_size,), device=self.device)
        else:
            sequences[:, 0] = torch.randint(0, self.vocab_size, (batch_size,), device=self.device)
    ...
    def get_color_range(self, color_idx: int) -> Tuple[int, int]:
        ...
        return (self.boundaries[color_idx].item(), 
