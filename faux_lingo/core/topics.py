...
class TopicModel:
    ...
    def sample_topic_modes(self) -> None:
        ...
        for _ in range(self.config.num_topics):
            ...
            for color in range(self.num_colors):
                ...
                mode_words = set(self._np_rng.choice(
                    words_in_color,
                    size=self.config.modes_per_color,
                    replace=False
                ))
    ...
    def build_topic_matrices(self) -> None:
        ...
        logger.debug("Built transition matrices for {} topics", len(self.topic_matrices))
    ...
    def compute_topic_distributions(self) -> None:
        ...
        logger.debug("Computed steady state distributions for {} topics", len(self.topic_distributions))
    ...
    def get_topic_entropy(self, topic_idx: int) -> Tuple[float, float]:
        ...
        stationary_entropy = -np.sum(
            pi[pi > 0] * np.log2(pi[pi > 0])
        )
