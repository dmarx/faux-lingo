...
class InfoTheoryAnalyzer:
    ...
    def compute_transition_entropy(
        self, transition_matrix: Float[torch.Tensor, "batch vocab vocab"]
    ...
    def compute_color_entropy(
        self, sequences: Int[torch.Tensor, "batch seq_len"], window_size: int = 2
    ) -> float:
        ...
        counts = torch.zeros(
            (self.generator.n_colors,) * window_size, device=sequences.device
        )
    ...
    def estimate_mutual_information(
        ...
        token_freqs = torch.zeros(
            (len(sequences), self.generator.vocab_size), device=sequences.device
        )
        for b in range(len(sequences)):
    ...
    def _mutual_info_discrete(self, X: torch.Tensor, Y: torch.Tensor) -> float:
        ...
        """
        # Joint distribution
        joint = torch.zeros((X.max() + 1, Y.max() + 1), device=X.device)
        for x, y in zip(X, Y):
            joint[x, y] += 1
    ...
    def analyze_sequence_entropy(
        ...
        sequences, mixtures = self.generator.sample_sequences(
            batch_size=batch_size, seq_length=seq_length, temperature=temperature
        )
        ...
        transition_entropy = (
            self.compute_transition_entropy(trans_matrices).mean().item()
        )

        color_entropy = self.compute_color_entropy(sequences, window_size=window_size)
        ...
        mutual_info = self.estimate_mutual_information(sequences, mixtures, n_bins=10)
...
if __name__ == "__main__":
    ...
    for temp in [0.5, 1.0, 2.0]:
        metrics = analyzer.analyze_sequence_entropy(batch_size=1000, temperature=temp)
        print(f"\nTemperature {temp}:")
