...
import numpy as np
import pytest
from faux_lingo.core.topics import TopicModel, TopicConfig
...
def test_topic_distribution_computation(word_colors, base_matrix):
    ...
    for dist in model.topic_distributions:
        ...
        modes = {word for mode_set in model.topic_modes[0].values() 
                for word in mode_set}
...
def test_topic_entropy_computation(word_colors, base_matrix):
    ...
    base_stationary = -np.sum(
        model.topic_distributions[0] * 
        np.log2(model.topic_distributions[0] + 1e-12)
    )
...
def test_deterministic_generation(word_colors, base_matrix):
    ...
    assert all(np.array_equal(m1, m2) 
